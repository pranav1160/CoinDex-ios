...
class CoinDetailViewModel:ObservableObject{
    ...
    private func createOverviewArray(coinModel: Coin) -> [Statistic] {
        let price = coinModel.currentPrice.asCurrencyWith2to6Decimals()
    ...
    private func createAdditionalArray(coinDetailModel: CoinDetail?, coinModel: Coin) -> [Statistic] {
        ...
        let high = coinModel.high24H?.asCurrencyWith2to6Decimals() ?? "n/a"
        let highStat = Statistic(title: "24h High", value: high)
        
        let low = coinModel.low24H?.asCurrencyWith2to6Decimals() ?? "n/a"
        let lowStat = Statistic(title: "24h Low", value: low)
        
        let priceChange = coinModel.priceChange24H?.asCurrencyWith2to6Decimals() ?? "n/a"
        let pricePercentChange = coinModel.priceChangePercentage24H
